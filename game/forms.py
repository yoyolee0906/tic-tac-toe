...
class NewGameForm(forms.Form):
	player1 = forms.CharField(max_length=64, required=True, validators=[validate_player_type])
	player2 = forms.CharField(max_length=64, required=True, validators=[validate_player_type])

	def create(self):
		players = [self.cleaned_data['player1'], self.cleaned_data['player2']]
		random.shuffle(players)
		return Game.objects.create(player_x=players[0], player_o=players[1])
